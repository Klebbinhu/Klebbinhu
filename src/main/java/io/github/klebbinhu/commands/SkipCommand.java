...
public class SkipCommand extends AbstractCommand {
    ...
    public SkipCommand(KlebbinhuBot klebbinhuBot) {
        this.musicController = klebbinhuBot.getMusicController();
    }

    @Override
    protected SlashCommandData getCommand() {
        return Commands.slash("skip", "Passa a música que está a tocar atualmente à frente, ou para se não houver próxima música.");
    }
    ...
    public void onCommand(SlashCommandInteractionEvent event) {
        ...
        MessageEmbed embed = new EmbedBuilder()
                ...
                .setDescription("Música **" + playing.getInfo().title + "** passada com sucesso!")
                .build();
        event.getHook().editOriginalEmbeds(embed).queue();
